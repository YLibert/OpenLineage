...
class DbtBaseProcessor:
    ...
    def parse(self) -> DbtEvents:
        """
            Parse dbt manifest, run_results, and catalog, and produce OpenLineage events.
        """
        manifest = self.artifacts['manifest']
        self.manifest_version = self.get_schema_version(manifest)

        run_results = self.artifacts['run_results']
        self.run_metadata = run_results['metadata']
        self.command = run_results['args']['which']

        if 'catalog' in self.artifacts.keys():
        ...
        context = DbtRunContext(manifest, run_results, catalog)
    ...
    def load_metadata(
        ...
        with open(path, 'r') as f:
            ...
            self.verify_schema_version(desired_schema_versions, metadata)
            return metadata

    def verify_schema_version(self, desired_schema_versions: List[int], metadata):
        str_schema_version = get_from_nullable_chain(
                metadata,
                ['metadata', 'dbt_schema_version']
            )
        schema_version = self.get_schema_version(metadata)
        if schema_version not in desired_schema_versions:
            if schema_version > max(desired_schema_versions):
                self.logger.warning(
                    f"Artifact schema version: {str_schema_version} is above "
...
class DbtArtifactProcessor(DbtBaseProcessor):
    def __init__(
        ...
        manifest = self.load_metadata(self.manifest_path, [2, 3, 4, 5, 6, 7])
        self.artifacts['manifest'] = manifest
        run_result = self.load_metadata(self.run_result_path, [2, 3, 4, 5])
        self.artifacts['run_result'] = run_result
        try:
            catalog: Optional[Dict[Any, Any]] = self.load_metadata(
                self.catalog_path, [1]
            )
            self.artifacts['catalog'] = catalog
...
# artifacts - is the map that has to contain
# connection, manifest, run_result, and catalog
class DbtCloudArtifactProcessor(DbtBaseProcessor):
    def __init__(
        self,
        producer: str,
        job_namespace: str,
        ...
        # the project - required by the
        connection = artifacts['connection']

        # get the adapter type
        try:
            self.adapter_type = Adapter[connection['type'].upper()]
        except KeyError:
            raise NotImplementedError(
                f"Only {Adapter.adapters()} adapters are supported right now. "
                f"Passed {connection['type']}"
            )
